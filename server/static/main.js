...
function viewJobDetails(job){
  var storageItem = JSON.stringify(job);
  localStorage.setItem("jobDetail", storageItem);
  window.location.href = `./get_job.html?${job.id}`;
};

function editJobDetails(job){
  var storageItem = JSON.stringify(job);
  localStorage.setItem("jobDetail", storageItem);
  window.location.href = `./update_data.html?${job.id}`;
};

function deleteJob(job){
  var storageItem = JSON.stringify(job);
  localStorage.setItem("jobDetail", storageItem);
  window.location.href = `./delete_job.html?${job.id}`;
};


const fetchData = async () => {
  var DataAPI = "http://127.0.0.1:5000/jobs";
        ...
        .map((job) => {
          ...
          <div class="listing-wrapper" id="listing">
            <div class="job-details">
              ...
              <div class="role">
                <button onclick='viewJobDetails(${JSON.stringify(job)})' class="writing">View<button>
                <button onclick='editJobDetails(${JSON.stringify(job)})' class="writing">Edit</button>
                <button onclick='deleteJob(${JSON.stringify(job)})' class="writing">Delete</button>
              </div>
            </div>
          </div>  
            `;
